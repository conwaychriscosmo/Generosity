class GiftsController < ApplicationController
  ...
  def rate
    output = Gift.rate(params[:gift][:rating],params[:sessions][:username])
    render json: output
  end
  
  #the giver delivers the gift
  def deliver
    output = Gift.deliver(params[:sessions][:username])
    render json: output
  end
  # POST /gifts
  # POST /gifts.json
  def create
    ...
    output = Gift.create(gift_params[:name], gift_params[:url], params[:sessions][:username])
    render json: output
